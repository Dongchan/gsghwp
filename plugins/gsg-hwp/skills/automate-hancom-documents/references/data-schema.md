...
이 플러그인의 `apply` 입력은 `text_fields`와 양식에 미리 정의된 한컴 네이티브 필드를 사용한다. 별도 `hwp_template_automation` 도구의 `fields` 키와는 호환되지 않는다. 본문에 적힌 `{{사업명}}` 같은 자리표시자를 네이티브 필드로 자동 변환하지 않으므로, 적용 전에 대상 HWP/HWPX 양식에 네이티브 필드를 만들어야 한다.

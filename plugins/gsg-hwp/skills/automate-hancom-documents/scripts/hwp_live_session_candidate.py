...
def select_operation_document(
    ...
    editable = tuple(
        ...
        if document.edit_mode == 1 and document.active
    )
    if requested:
        matches = tuple(
            document
        ...
        if len(matches) == 1:
            return matches[0]
        raise HwpLiveError(
            "hwp_operate의 document와 일치하는 활성 편집 문서를 정확히 하나 찾을 수 없습니다"
        )
    if len(editable) == 1:
        return editable[0]
    if not editable:
        raise HwpLiveError("활성 상태인 편집 가능 한컴 문서가 없습니다")
    raise HwpLiveError(
        "활성 한컴 문서가 여러 개입니다. hwp_operate의 document에 전체 경로를 지정하세요"
    )

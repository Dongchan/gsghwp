...
MCP_TOOL_SPECS: Final = (
    ...
    McpToolSpec("hwp_inspect", "connect나 session_id 선행 작업 없이 현재 쪽·마지막 한컴 커서·선택 텍스트·선택 셀 블록·선택 표와 글자·문단·용지 모양을 읽습니다. active_target은 현재 또는 포커스 이동 직전 마지막 한컴 위치를 뜻합니다.", "state", "read", "com_dispatch", requires_session=False, profiles=_ALL_PROFILES),
    ...
    McpToolSpec("hwp_list_window_states", "ROT 문서가 없어도 현재 보이는 한/글 창과 시작 오류 팝업의 핸들·제목·클래스·소유 관계를 나열합니다.", "diagnostic", "read", "win32_ui", requires_session=False, profiles=_ALL_PROFILES),
    ...
    McpToolSpec("hwp_insert_layout", HWP_INSERT_LAYOUT_DESCRIPTION, "layout", "write", "native_required", profiles=_ALL_PROFILES, workflow="document.insert_layout"),

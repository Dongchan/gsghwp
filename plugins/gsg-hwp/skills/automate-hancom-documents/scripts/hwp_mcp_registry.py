...
MCP_TOOL_SPECS: Final = (
    ...
    McpToolSpec(
        ...
        "원본 이미지를 한 번 분석하고 100KB 이하 compact 구조 요약, ordinary_layout/reference_layout_bulk 추천과 이유, 검증 좌표, 바로 전달 가능한 reference_layout 초안을 반환합니다. 대용량 검출 자료와 이미지 산출물은 로컬 파일에 두며 HWP를 변경하지 않습니다.",
        "image",
        "read",
        "python_catalog",
        requires_session=False,
    ...
    McpToolSpec(
        "hwp_list_styles",
        "connect나 session_id 없이 현재 문서의 문단 스타일 이름과 ID를 읽습니다.",
        "style",
        "read",
        "com_dispatch",
        requires_session=False,
